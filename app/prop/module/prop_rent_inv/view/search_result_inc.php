							...
							<tbody>
								...
								if ($arr_count_general_model >0){
									...
									foreach ($arr_general_model as $general_model): 
										...
										echo '<td>'.$i_count++.'</td>';
										echo '<td>';
										echo '<a href="'.actionURL('edit','?item_id='.$general_model[$dmGeneralModel->primary_keyname()].'&lot_id='.$lot_id.'&page='.$page).'">'.f_html_escape($general_model['inv_code']).'&nbsp;'.'&raquo;'.'</a>';
										echo '</td>';

										echo '<td>'.f_html_escape(YMDtoDMY($general_model['inv_date'])).'</td>';						
										...
										echo '<td style="text-align:right" >'.f_html_escape(number_format($general_model['balance'],2)).'</td>';

<?php
class article_season_master_model extends dataManager
{
	private $dbh;
	private $primary_table;
	private $primary_keyname;
	private $primary_indexname;

	private $table_field;  // variable for dataManager
	private $errorMsg;   // variable for dataManager
	private $mainTable;   // variable for dataManager
	
	
	public function __construct()
    {
		parent::__construct();
    	$this->mainTable='tbl_article_season_master';
    	$this->setTable($this->mainTable);
    	$this->setErrorMsg('Article -> Maintenance -> Season Master -> SQL error:');
    	$this->table_field=$this->getTableField();

		$this->primary_keyname = 'season_id';
		$this->primary_indexname = 'season_id';
		try {
			$this->dbh = new PDO(DB_CONNECTION_STRING,DB_USERNAME,DB_PASSWORD);
			$this->dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$this->dbh->query("set names utf8");
			} catch (PDOException $e) {
				print 'Error!: ' . $e->getMessage();
				die();
				}
	}
	
   public function primary_keyname()
	{
		return $this->primary_keyname;
	}

   public function search($jsondata)
	{
		$json = json_decode($jsondata, true);
	
		$sql_filter = "";
		
		
		if($json['general']['name']<>"") {
			if(!empty($sql_filter)) $sql_filter.=" AND ";
			$sql_filter .= " CAT.name LIKE '%".addslashes($json['general']['name'])."%'" ;
		}

		//echo "<br>sql_filter:".$sql_filter."<br>";
		
		
		$sql = "SELECT "."CAT.".$this->primary_keyname. " FROM tbl_article_season_master AS CAT ";
		$sql .= " WHERE ";		
		
		$sql .= " (1) " ;
		if(!empty($sql_filter)) $sql .= " AND  ".$sql_filter ;
		$sql .= " ORDER BY ". $this->primary_indexname . "  DESC ".  ";";
		//echo "<br>sql:".$sql."<br>";
	
		$arr_primary_id =array();
		$rows = $this->runSQLAssoc($sql);	
		foreach ($rows as $row): 
			$arr_primary_id[] = "'". addslashes($row[$this->primary_keyname]) ."'";
		endforeach; 
		

		$array_count = count($arr_primary_id);

		if ($array_count > 0){	  
			$result_id = implode(",", $arr_primary_id);
		}
		
		$lot_id = strtotime(date("Y-m-d H:i:s")).rand(0, 10);;

		$sql = 'INSERT INTO `tbl_sys_paging_control`(
					`searchphrase`,
					`lot_id`,
					`result_id`,
					`create_user`,
					`create_datetime`
					) VALUES (';
		$sql.='\''.addslashes($jsondata).'\''.',';
		$sql.='\''.addslashes($lot_id).'\''.',';
		$sql.='\''.addslashes($result_id).'\''.',';
		$sql.='\''.addslashes($_SESSION["sUserID"]).'\''.',';
		$sql.='now()'.')';

		$last_insert_id = $this->runSQLReturnID($sql);		


		return $lot_id;
	}

 
  public function retreive_content($lot_id,$page)
	{

		$sql = "SELECT * FROM tbl_sys_paging_control WHERE lot_id = '".$lot_id."' AND create_user =	'".$_SESSION['sUserID']."';";
		//echo "<br>sql:".$sql."<br>";
		$arr_record = $this->runSQLAssoc($sql);	


		$arr =  $arr_record[0];
		$arr_primary_id = $arr['result_id']; 
		
		
		if ($arr_primary_id != '')		
		{

			
			$sql = "SELECT CAT.* FROM tbl_article_season_master AS CAT ";
		
			if(!empty($arr_primary_id)) $sql .= "WHERE "."CAT.".$this->primary_keyname. " in (".$arr_primary_id.")" ;
			$sql .= " ORDER BY CAT." .$this->primary_indexname. ' DESC ' ;
			$sql .= " LIMIT ". SYSTEM_PAGE_ROW_LIMIT . " OFFSET  ".($page-1)*SYSTEM_PAGE_ROW_LIMIT ;
			//echo "<br>sql:".$sql."<br>";
			$arr_record = $this->runSQLAssoc($sql);	

				
			return $arr_record;
		}
		
		return $arr_record;
	}

  public function paging_config($lot_id)
	{
		$sql = "SELECT * FROM tbl_sys_paging_control WHERE lot_id = '".$lot_id."' AND create_user =	'".$_SESSION['sUserID']."';";
		//echo "<br>sql:".$sql."<br>";
		$arr_record = $this->runSQLAssoc($sql);	


		$result_id = $arr_record[0]['result_id'];

		//$this->dbh->beginTransaction();

		$sql ="UPDATE `tbl_sys_paging_control` SET modify_datetime =now()	WHERE lot_id ='$lot_id'";

		$void = $this->runSQLReturnID($sql);


		return $result_id;
	}	

    public function searchphrase($lot_id)
	{
		$sql = "SELECT searchphrase FROM tbl_sys_paging_control WHERE lot_id = '".$lot_id."' AND create_user =	'".$_SESSION['sUserID']."';";
		//echo "<br>sql:".$sql."<br>";
		$arr_record = $this->runSQLAssoc($sql);	
		
		$searchphrase = $arr_record[0]['searchphrase'];
		return $searchphrase;
				
	}		

    public function select($primary_id)
	{
 		$sql ="SELECT * FROM tbl_article_season_master WHERE ".$this->primary_keyname. " = '$primary_id'";
		//echo "<br>sql:".$sql."<br>";
		$arr_record = $this->runSQLAssoc($sql);	

		return $arr_record[0];
	}		
	
 
	public function create($general)
	{
		$name = $general['name'];
		$date_from = toYMD($general['date_from']);
		$date_to = toYMD($general['date_to']);
		$content = $general['content'];
		$publish_is = 0;
		$status = 1;
		$create_user = $_SESSION['sUserID'];		

		$ar_fields=array();
		$ar_fields['name'] = $name;
		$ar_fields['date_from'] = $date_from;
		$ar_fields['date_to'] = $date_to;
		$ar_fields['content'] = $content;
		$ar_fields['publish_is'] = $publish_is;
		$ar_fields['status'] = $status;
		$ar_fields['create_user'] = $create_user;
		$ar_fields['modify_user'] = $create_user;
		$ar_fields['create_datetime'] = 'now()';
		$ar_fields['modify_datetime'] = 'now()';

		$sql= $this->createInsertSql($ar_fields,$this->table_field,$this->mainTable);
		//echo '<br> sql : '.$sql.'<br>';
		$last_insert_id = $this->runSQLReturnID($sql);

		return $last_insert_id;
	}
	
	
	public function update($primary_id, $general)
	{
		$name = $general['name'];
		$date_from = toYMD($general['date_from']);
		$date_to = toYMD($general['date_to']);
		$content = $general['content'];
		$publish_is = $general['publish_is'];
		$status = $general['status'];
		$modify_user = $_SESSION['sUserID'];

		$ar_fields=array();
		$ar_fields['name'] = $name;
		$ar_fields['date_from'] = $date_from;
		$ar_fields['date_to'] = $date_to;
		$ar_fields['content'] = $content;
		$ar_fields['publish_is'] = $publish_is;
		$ar_fields['status'] = $status;
		$ar_fields['modify_user'] = $modify_user;
		$ar_fields['modify_datetime'] = 'now()';

		$sql= $this->createUpdateSql($ar_fields,$this->table_field,$this->mainTable,$this->primary_keyname,$primary_id);
		//echo '<br> sqal : '.$sql.'<br>';
		$this->runSql($sql);


		
		return $last_insert_id;
	}	

	
	public function is_duplicate_field($field_name, $para)
	{
		$para = addslashes($para);
		$sql ="SELECT COUNT(*) AS RecordCount FROM tbl_article_season_master ";
		$sql .= "  WHERE ";
		$sql .= " (1) " ;
		$sql .=" AND $field_name = '$para'";
		
		//echo '<br>'.$sql; // Debug used		
		$arr_record = $this->runSQLAssoc($sql);	


		$is_find = false;
		if ($arr_record[0]['RecordCount'] >=1) $is_find = true;
		
		return $is_find;
	}	
			
			
	public function is_duplicate_field_myself($myself_id_para , $field_name, $field_para)
	{
		$myself_id_para = addslashes($myself_id_para);
		$field_name = addslashes($field_name);
		$field_para = addslashes($field_para);
		
		$sql ="SELECT COUNT(*) AS RecordCount FROM tbl_article_season_master ";
		$sql .= "  WHERE ";
		$sql .= " (1) " ;
		$sql .=" AND $field_name = '$field_para' AND ".$this->primary_keyname. "<>'$myself_id_para' ";

		//echo '<br>'.$sql; // Debug used		
		$arr_record = $this->runSQLAssoc($sql);	


		$is_find = false;
		if ($arr_record[0]['RecordCount'] >=1) $is_find = true;
		
		return $is_find;
	}	
		
	
    public function subcategory_list($primary_id)
	{

		$sql = "SELECT SUB.*  FROM tbl_article_season_master_sub AS SUB  ";
		$sql .= " LEFT JOIN tbl_article_season_master AS C  ON SUB.season_id = C.season_id  ";
		$sql .= " WHERE SUB.".$this->primary_keyname. " = '$primary_id'";
		$sql .= " ORDER BY SUB.sub_id; ";

		
		//echo "<br>sql:".$sql."<br>";
		$arr_record = $this->runSQLAssoc($sql);	


		return $arr_record;
	}		




	public function subcategory_create($primary_id, $general)
	{
		
		$name = $general['name'];
		$status = 1;
		$create_user = $_SESSION['sUserID'];		

		$ar_fields=array();
		$ar_fields['season_id'] = $primary_id;
		$ar_fields['name'] = $name;
		$ar_fields['status'] = $status;
		$ar_fields['create_user'] = $create_user;
		$ar_fields['modify_user'] = $create_user;
		$ar_fields['create_datetime'] = 'now()';
		$ar_fields['modify_datetime'] = 'now()';

		
		$this->mainTable='tbl_article_season_master_sub';
    	$this->setTable($this->mainTable);
       	$this->table_field=$this->getTableField();

		$sql= $this->createInsertSql($ar_fields,$this->table_field,$this->mainTable);
		echo '<br> sql : '.$sql.'<br>';
		$last_insert_id = $this->runSQLReturnID($sql);	

		return true;
	}	


	
    public function subcategory_select($sub_id)
	{
 		$sql ="SELECT * FROM tbl_article_season_master_sub WHERE ".'sub_id'. " = '$sub_id'";
		//echo "<br>sql:".$sql."<br>";
		$arr_record = $this->runSQLAssoc($sql);	

		return $arr_record[0];
	}		
	
	
	public function subcategory_update($sub_id, $general)
	{
		$name = $general['name'];
		$status = $general['status'];
		$modify_user = $_SESSION['sUserID'];

		$ar_fields=array();
		$ar_fields['name'] = $name;
		$ar_fields['status'] = $status;
		$ar_fields['modify_user'] = $modify_user;
		$ar_fields['modify_datetime'] = 'now()';

		$this->mainTable='tbl_article_season_master_sub';
    	$this->setTable($this->mainTable);
       	$this->table_field=$this->getTableField();

		$sql= $this->createUpdateSql($ar_fields,$this->table_field,$this->mainTable,'sub_id',$sub_id);
		//echo '<br> sqal : '.$sql.'<br>';
		$this->runSql($sql);

		
		return $void;
	}	

	
	
    public function headline_list($primary_id)
	{
		
		$sql = "SELECT H.*, M.name as 'category_name'   FROM tbl_article_season_master_headline AS H  ";
		$sql .= " LEFT JOIN tbl_article_category_master AS M  ON H.cate_id = M.cate_id  ";
		$sql .= " WHERE H.".$this->primary_keyname. " = '$primary_id'";
		$sql .= " ORDER BY H.season_headline_id; ";
		
		//echo "<br>sql:".$sql."<br>";
		$arr_record = $this->runSQLAssoc($sql);	
		

		return $arr_record;
	}		

	
    public function article_category_master_viewall()
	{
		$sql ="SELECT * FROM tbl_article_category_master ORDER BY cate_id ;";
		//echo '<br>'.$sql; // Debug used		
		$arr_record = $this->runSQLAssoc($sql);	


		return $arr_record;
	}		
	
	
	public function headline_create($primary_id, $general)
	{
		
		$season_id = addslashes($general['season_id']);
		$cate_id = addslashes($general['cate_id']);
		$headline = addslashes($general['headline']);
		$status = 1;
		$create_user = $_SESSION['sUserID'];
	
		
		$sql = "INSERT INTO `tbl_article_season_master_headline`(
						`season_id`
						,`cate_id`
						,`headline`
						,`status`
						,`create_user`
						,`modify_user`
						,`create_datetime`
						,`modify_datetime`
						) VALUES (
							'$primary_id'
							,'$cate_id'
							,'$headline'
							,'$status'
							,'$create_user'
							,'$create_user'
							,now()
							,now()
							)";
		
		//echo '<br>'.$sql; // Debug used				
		$void = $this->runSQLReturnID($sql);

		
		return $last_insert_id;
	}	

	
	
    public function headline_select($season_headline_id)
	{
 		$sql ="SELECT * FROM tbl_article_season_master_headline WHERE ".'season_headline_id'. " = '$season_headline_id'";
		//echo "<br>sql:".$sql."<br>";
		$arr_record = $this->runSQLAssoc($sql);	


		return $arr_record[0];
	}		
	
	
	
	public function headline_update($season_headline_id, $general)
	{
		$cate_id = addslashes($general['cate_id']);
		$headline = addslashes($general['headline']);
		$status = $general['status'];
		$modify_user = $_SESSION['sUserID'];

	
		$sql ='UPDATE  `tbl_article_season_master_headline` SET ';
		$sql.='`cate_id`='.'\''.$cate_id.'\'';
		$sql.=',`headline`='.'\''.$headline.'\'';
		$sql.=',`status`='.'\''.$status.'\'';
		$sql.=',`modify_user`='.'\''.$modify_user.'\'';
		$sql.=',`modify_datetime`=NOW()'.' ';
		$sql.=' WHERE ';
		$sql.='`'.'season_headline_id'. '`='.'\''.addslashes($season_headline_id).'\''.' ';
		//echo '<br>'.$sql; // Debug used				
		$void = $this->runSQLReturnID($sql);

		
		return $last_insert_id;
	}	
	
	
	
    public function close()
	{
		$this->dbh = null;
	}	
	
} //class
?>
<?php
class article_category_master_model extends dataManager
{
	private $dbh;
    ...
    {
		parent::__construct();
    	$this->mainTable='tbl_article_category_master';
    	$this->setTable($this->mainTable);
    	$this->setErrorMsg('Article -> Maintenance -> Category Master -> SQL error:');
    	$this->table_field=$this->getTableField();
	
		$this->primary_keyname = 'cate_id';
		$this->primary_indexname = 'cate_id';
		try {
   ...
   public function search($jsondata)
	{
		...
		$sql = "SELECT "."CAT.".$this->primary_keyname. " FROM tbl_article_category_master AS CAT ";
		$sql .= " WHERE ";		
  ...
  public function retreive_content($lot_id,$page)
	{
		...
		if ($arr_primary_id != '')		
		{
			...
			$sql = "SELECT CAT.* FROM tbl_article_category_master AS CAT ";
  ...
  public function paging_config($lot_id)
	...
	{
 		$sql ="SELECT * FROM tbl_article_category_master WHERE ".$this->primary_keyname. " = '$primary_id'";
		//echo "<br>sql:".$sql."<br>";
		$arr_record = $this->runSQLAssoc($sql);	
	...
	{
		$name = $general['name'];
		$sorting = $general['sorting'];
		$status = 1;
		$create_user = $_SESSION['sUserID'];		

		$ar_fields=array();
		$ar_fields['name'] = $name;
		$ar_fields['sorting'] = $sorting;
	...
	public function is_duplicate_field($field_name, $para)
	{
		...
		$sql ="SELECT COUNT(*) AS RecordCount FROM tbl_article_category_master ";
		$sql .= "  WHERE ";
		$sql .= " (1) " ;

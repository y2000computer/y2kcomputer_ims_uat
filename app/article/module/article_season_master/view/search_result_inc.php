					...
					<div class="sidebarContentCol" id="TransactionsDiv">
						...
						<table class="searchResult" border="0" cellspacing="0" cellpadding="0">
							<tbody>
								...
								<th style="text-align:left" width ="10%'">Date From</th>
								<th style="text-align:left" width ="10%'">Date To</th>
								...
								if ($arr_count_general_model >0){
									...
									foreach ($arr_general_model as $general_model): 
										...
										echo '<td>'.$i_count++.'</td>';
										echo '<td>';
										echo '<a href="'.actionURL('edit','?item_id='.$general_model[$dmGeneralModel->primary_keyname()].'&lot_id='.$lot_id.'&page='.$page).'">'.htmlspecialchars($general_model['name']).'&nbsp;'.'&raquo;'.'</a>';
										echo '</td>';

										echo '<td>'.toDMY($general_model['date_from']).'</td>';
										echo '<td>'.toDMY($general_model['date_to']).'</td>';
										echo '<td>';		
